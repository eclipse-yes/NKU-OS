...
void copyDir(char* dst_path, char* src_path) {
    ...
    while ((cur_node = readdir(src)) != NULL) {
        ...
        } else if (cur_node->d_type == DT_LNK) {
        // Step2: copy symbolic link files.
          	// Use link function to copy symbolic link file.
            int res = link(new_src_path, new_dst_path);
            if (res != 0)
                printf("FAILURE: link([oldpath]%s, [newpath]%s)\n", new_src_path, new_dst_path);
        } else {
